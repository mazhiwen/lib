...
//gulp.task('sass', function () {
  //return gulp.src(fileName+'/**/*.scss',{base:fileName}).pipe(sass.sync().on('error', sass.logError)).pipe(gulp.dest('/dist'));
//});

gulp.task('sass', function () {
  return gulp.src(locationBefore+fileName+'/**/*.scss',{base:locationBefore+fileName}).pipe(sass.sync().on('error', sass.logError)).pipe(gulp.dest('./dist'+'/'+fileName));
});
